...
const calculateMatchPoints = (predHome, predAway, realHome, realAway) => {
  ...
  return predTrend === realTrend ? 1 : 0; 
};

const formatDisplayDate = (dateVal) => {
  try {
    if (!dateVal || (typeof dateVal === 'string' && dateVal.toLowerCase().includes('definir'))) {
      return "Fecha a confirmar";
    }

    const d = dateVal instanceof Date ? dateVal : new Date(dateVal);
    
    // Si la fecha es inválida, devolvemos el texto crudo
    if (isNaN(d.getTime())) return typeof dateVal === 'string' ? dateVal : "Fecha a confirmar";

    // Formato elegante: "11/06" (SOLO DÍA Y MES)
...
export default function MatchCard({ match, prediction, onPredictionChange, onSavePrediction, stats, activeJokerMatchId }) { 
  ...
  // Manejo de Tiempos y Bloqueos
  const displayDate = formatDisplayDate(match.date || match.rawDate);
  
  const isLockedByTime = React.useMemo(() => {
    if (match.date instanceof Date && !isNaN(match.date.getTime())) {
       const cutoffTime = new Date(match.date.getTime() - 60 * 60 * 1000); // 60 mins antes
  ...
  // Estilos Condicionales Dinámicos
  const getCardStyles = () => {
    if (isJoker && !isFinished) return "bg-slate-900 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.15)]";
    if (isFinished) return "bg-slate-900/50 border-slate-800 opacity-75";
    if (match.group?.toUpperCase().includes('FINAL')) return "bg-gradient-to-br from-slate-900 via-slate-900 to-amber-900/30 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.15)] hover:border-amber-400 scale-[1.02]";
    return "bg-slate-900 border-slate-700 hover:border-emerald-500/50";
  };

  const btnProps = {
    text: !hasValues ? "Confirmar Pronóstico" : isDirty ? "Actualizar Pronóstico 🔄" : "Pronóstico Guardado ✅",
    style: !hasValues 
  ...
  const changeByButton = (type, increment) => {
    ...
    let newVal = Math.max(0, Math.min(15, val + increment)); // Limita entre 0 y 15 limpiamente
    onPredictionChange(match.id, type, newVal.toString());
  };

  const handleJokerToggle = () => {
    if (isLocked) return;
    if (!canToggleJoker) return alert("⚠️ Ya utilizaste tu comodín en otro partido de este grupo. Primero desactívalo allí.");
  ...
  const handleShare = async () => {
    ...
    // Pequeño retraso para que el DOM se actualice (oculte los botones)
    setTimeout(async () => {
      try {
        const dataUrl = await toPng(cardRef.current, { cacheBust: true, backgroundColor: '#0f172a', pixelRatio: 2 });
  ...
  // ==========================================
  return (
    <div ref={cardRef} className={`p-5 rounded-2xl border relative overflow-hidden transition-all duration-300 ${getCardStyles()}`}>
          ...
          <button 
            ...
            className={`px-3 py-1.5 text-[10px] font-bold rounded-br-lg flex items-center gap-1 transition-all ${
              isJoker 
                ...
                : canToggleJoker ? 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-amber-400' : 'bg-slate-900/80 text-slate-600 cursor-not-allowed'
            }`}
            title={isJoker ? "Comodín Activado" : "Activar Comodín"}
          >
            <Zap className="w-3 h-3" fill={isJoker ? "currentColor" : "none"} /> {isJoker ? 'x2 Pts' : 'Comodín'}
          </button>
          
          <button onClick={() => setShowInfoPopup(true)} className="bg-slate-800/80 text-slate-400 px-1.5 py-1.5 rounded-br-lg hover:text-blue-400 transition-colors">
      ...
      {/* Cabecera Grupo/Fecha */}
      <div className={`text-xs mb-4 mt-2 text-center uppercase ${match.group?.toUpperCase().includes('FINAL') ? 'text-amber-500 font-black tracking-widest' : 'text-slate-500'}`}>
        {match.group} <br/> <span className="text-[10px] text-slate-400">{displayDate}</span>
      </div>
      
      {/* Área de Banderas e Inputs */}
      <div className="flex items-center justify-between mb-5">
        {/* Equipo Local */}
        <div className="flex flex-col items-center w-[30%]">
          <div className="mb-3 transition-transform hover:scale-110 flex justify-center">
            {match.flagH && match.flagH.length >= 2 && match.flagH !== 'UN'  
        ...
        <div className="flex items-center gap-2 w-[40%] justify-center">
          ...
          <div className={`flex items-center bg-slate-950/80 border ${isLocked ? 'border-red-900/50' : 'border-slate-600 focus-within:border-emerald-500'} rounded-xl overflow-hidden transition-colors shadow-inner`}>
            {isCapturing ? (
              <div className="w-10 h-14 flex items-center justify-center text-2xl font-black text-white">{myPred.homeScore}</div>
            ) : (
              <input type="number" disabled={isLocked} value={myPred.homeScore ?? ''} onChange={(e) => handleScoreChange('homeScore', e.target.value)} className={`w-10 h-14 bg-transparent text-center text-2xl font-black text-white outline-none appearance-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${isLocked && 'opacity-50 text-slate-500'}`} />
            )}
            
            {!isLocked && !isCapturing && (
              <div className="flex-col border-l border-slate-700/50 h-full flex">
                <button onClick={() => changeByButton('homeScore', 1)} className="flex-1 px-1.5 hover:bg-slate-800 text-slate-400 hover:text-emerald-400 transition-colors border-b border-slate-700/50 flex justify-center"><ChevronUp className="w-4 h-4" strokeWidth={3} /></button>
                <button onClick={() => changeByButton('homeScore', -1)} className="flex-1 px-1.5 hover:bg-slate-800 text-slate-400 hover:text-emerald-400 transition-colors flex justify-center"><ChevronDown className="w-4 h-4" strokeWidth={3} /></button>
              </div>
            )}
          </div>
          ...
          <div className={`flex items-center bg-slate-950/80 border ${isLocked ? 'border-red-900/50' : 'border-slate-600 focus-within:border-emerald-500'} rounded-xl overflow-hidden transition-colors shadow-inner`}>
            {isCapturing ? (
              <div className="w-10 h-14 flex items-center justify-center text-2xl font-black text-white">{myPred.awayScore}</div>
            ) : (
              <input type="number" disabled={isLocked} value={myPred.awayScore ?? ''} onChange={(e) => handleScoreChange('awayScore', e.target.value)} className={`w-10 h-14 bg-transparent text-center text-2xl font-black text-white outline-none appearance-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${isLocked && 'opacity-50 text-slate-500'}`} />
            )}
            
            {!isLocked && !isCapturing && (
              <div className="flex-col border-l border-slate-700/50 h-full flex">
                <button onClick={() => changeByButton('awayScore', 1)} className="flex-1 px-1.5 hover:bg-slate-800 text-slate-400 hover:text-emerald-400 transition-colors border-b border-slate-700/50 flex justify-center"><ChevronUp className="w-4 h-4" strokeWidth={3} /></button>
                <button onClick={() => changeByButton('awayScore', -1)} className="flex-1 px-1.5 hover:bg-slate-800 text-slate-400 hover:text-emerald-400 transition-colors flex justify-center"><ChevronDown className="w-4 h-4" strokeWidth={3} /></button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col items-center w-[30%]">
          <div className="mb-3 transition-transform hover:scale-110 flex justify-center">
            {match.flagA && match.flagA.length >= 2 && match.flagA !== 'UN'  
      ...
      {/* Área de Resolución y Acciones Inferiores */}
      {!isCapturing && (
        <div>
          {!isLocked ? (
            <button onClick={handleSave} className={`w-full py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 border ${btnProps.style}`}>
              {btnProps.text.includes('✅') && <CheckCircle className="w-5 h-5" />} {btnProps.text}
            </button>
          ) : isFinished ? (
      ...
      {/* Banner inferior de Estado al Capturar (Screenshot) */}
      {isCapturing && (
        <div className={`mt-4 pt-3 border-t ${isJoker ? 'border-amber-900/50 bg-amber-950/20' : 'border-emerald-900/50 bg-emerald-950/20'} text-center rounded-xl pb-3 border`}>
        ...
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-slate-900/90 backdrop-blur-sm p-4" onClick={() => setShowInfoPopup(false)}>
          <div className="bg-slate-800 border border-amber-500 text-white p-6 rounded-2xl shadow-2xl animate-in zoom-in-50 duration-300">
            ...
            <p className="text-sm text-slate-300 text-center">
              Si activás el comodín y acertás, <b>tus puntos se multiplicarán x2</b>.<br/><br/> Solo uno por grupo.
            </p>
